...
export default function Ecommerce() {
  ...
  useEffect(() => {
    if (providerDetails === null) {
      router.push("/signin");
    } else if (providerDetails && !providerDetails.isApproved) {
  ...
  const pendingState = useMemo(() => {
    if (providerDetails && !isApproved) {
      ...
      if (!kycCompleted) {
        return {
          title: "KYC Pending",
          message: "Your store info is complete. Please upload your KYC documents.",
  ...
  return (
    <>
      ...
      {/* Show modal only if provider exists but not approved */}
